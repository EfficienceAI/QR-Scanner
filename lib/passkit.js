...
function errorMessage(data, status) {
  ...
  return `PassKit responded ${status}`;
}
...
async function request(method, path, body, opts = {}, attempt = 0) {
  ...
  if (!resp.ok) {
    ...
    throw new PassKitError(resp.status, errorMessage(data, resp.status), data, path);
  }
...
/**
 * Events for one member via the per-member route. Undocumented REST mapping
 * of the listEventsForMember RPC. It takes no paging, so PassKit returns at
 * most its default page (25 events, oldest first). Kept as a fallback only.
 */
...
/**
 * Events for a programme with PassKit filters + paging.
 * `filters` = { limit, offset, filterGroups: [{ condition, fieldFilters: [{ filterField, filterValue, filterOperator }] }] }
 */
async function listProgramEvents(programId, filters) {
  return request(
    'POST',
    `/members/program/list/events/${encodeURIComponent(programId)}`,
    { filters },
    { stream: true }
  );
}
